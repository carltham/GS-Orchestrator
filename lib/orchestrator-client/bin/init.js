...
function parseDockerPorts(dockerFilePath) {
  try {
    const content = fs.readFileSync(dockerFilePath, 'utf-8');
    const dbPortMatch = content.match(/["']?(\d+):5432["']?/);
    if (dbPortMatch && dbPortMatch[1]) {
      return parseInt(dbPortMatch[1], 10);
    }
  } catch (e) {}
  return null;
}
...
function searchProjectFiles(targetDir, depth = 0) {
  ...
  try {
    ...
    if (fs.existsSync(path.join(targetDir, 'docker-compose.yml'))) {
      ...
      dockerFilePath = path.join(targetDir, 'docker-compose.yml');
    } else if (fs.existsSync(path.join(targetDir, 'docker-compose.yaml'))) {
      result.dockerDir = targetDir;
      result.dockerFile = 'docker-compose.yaml';
      dockerFilePath = path.join(targetDir, 'docker-compose.yaml');
    } else if (fs.existsSync(path.join(targetDir, 'docker-compose.dev.yml'))) {
      result.dockerDir = targetDir;
      result.dockerFile = 'docker-compose.dev.yml';
      dockerFilePath = path.join(targetDir, 'docker-compose.dev.yml');
    }

    if (dockerFilePath) {
      result.dockerDbPort = parseDockerPorts(dockerFilePath);
    }

    // Check for Orchestrator server ONLY if running inside GS-Orchestrator repo
    if (
      fs.existsSync(path.join(targetDir, 'GS-Orchestrator', 'src', 'server.ts')) &&
      fs.existsSync(path.join(targetDir, 'GS-Orchestrator-GUI', 'angular.json'))
    ) {
    ...
    for (const entry of entries) {
      if (
        ...
        if (!result.dockerDir && subResult.dockerDir) {
          ...
          result.dockerDbPort = subResult.dockerDbPort;
        }
      }
    }
  } catch (e) {}
...
/**
 * Auto-generated Startup Handler Class for ${pkgName}
...
`;
  ...
  const setEnvVar = (key, value) => {
    const prefix = `${key}=`;
    const index = envLines.findIndex((line) => line.trim().startsWith(prefix));
    if (index >= 0) {
  ...
  if (detected.dockerDbPort) {
    setEnvVar('BASE_DATABASE_PORT', detected.dockerDbPort);
    setEnvVar('STRICT_PORT_ENFORCEMENT', 'true');
    console.log(`✅ Auto-detected docker database port ${detected.dockerDbPort} -> set in .env`);
  }

  fs.writeFileSync(envPath, envLines.join('\n').trim() + '\n', 'utf-8');
  console.log(`✅ Configured environment variables in ${envPath}`);
  ...
  if (fs.existsSync(pkgPath)) {
    try {
      ...
      pkg.scripts['start'] = 'node -r dotenv/config node_modules/@gs/orchestrator-client/dist/index.js || node startupHandler.js';
